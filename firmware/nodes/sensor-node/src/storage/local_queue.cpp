...
// ---------------------------------------------------------------------------
// Internal helpers — anonymous namespace (must precede local_queue namespace)
// ---------------------------------------------------------------------------
namespace {
...
static constexpr uint32_t kMagic    = 0x4E514D32;  // "NQM2"
static constexpr uint16_t kVersion  = 3;
static constexpr uint16_t kCapacity = 24;           // 24 × 124B = 2976B records
                                                     // + header + checksum = ~3004B < 4000B NVS limit
...
bool load() {
  ...
  if (n != sizeof(g_blob)) { p.end(); return false; }
...
// ---------------------------------------------------------------------------
// Public API — local_queue namespace
// ---------------------------------------------------------------------------
