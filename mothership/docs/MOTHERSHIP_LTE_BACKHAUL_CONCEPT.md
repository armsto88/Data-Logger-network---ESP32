...
- `ESP_PG` (GPIO35) is the power-good signal only
- `PS/SYNC` should be configured per the TPS63020 datasheet for the intended operating mode
...
- Longest SIM trace is currently ~53 mm; target ≤45 mm after layout adjustment
- SIM traces should be routed as a compact group with solid ground return underneath
- Add 100 nF (minimum) and optionally 1 µF decoupling on SIM_VDD **near the SIM socket**
- SIM traces must be kept away from: TPS63020 switching node, inductor, modem VBAT path, RF trace, USB D+/D−, SD card SPI clock, ESP32 RF antenna area
