...
| `VOLT` | `BATTERY_VSENSE` | ADC voltage divider midpoint |
...
| Ref | Part | Purpose |
|---|---|---|
| U52 | SN74LVC2G74DCUR | D-type flip-flop for config wake-reason latch; powered from `KEEP_ALIVE`; only `/PRE` and `/CLR` used |
| Main switch | AO3407A (P-ch MOSFET) | Main power switch: `RAW_BAT` → `VSYS`; gate pull-up to `RAW_BAT` |
| Input protection | AO3407A (P-ch MOSFET) | Reverse-polarity or ideal-diode path on battery input |
| Q41 | 2N7002 (N-ch MOSFET) | Low-side pull-down for config latch `/CLR` |
| Q42 | 2N7002 (N-ch MOSFET) | Level translator for `CONFIG_WAKE_PIN` sense |
...
| Ref | Part | Purpose |
|---|---|---|
| Main 3V3 reg | AP2112K-3.3 | Main switched 3.3 V regulator: `VSYS` → `3V3_SYS` |
| Keep-alive reg | TPL720F33 | Always-on 3.3 V regulator: `RAW_BAT` → `KEEP_ALIVE` |
| TPL720F33 | Always-on 3.3 V regulator: `RAW_BAT` → `KEEP_ALIVE` |
| CN3163 | Solar Li-ion charger: solar input → `BAT_BUS`, ~330 mA charge current |
| TPS63020 | Buck-boost regulator: `VSYS` → ~3.9 V modem rail |
| A7670G | LTE Cat-1 cellular modem module |
...
| Ref | Part | Purpose |
|---|---|---|
| USB-C connector | USB 2.0 device/sink | Programming, serial debug, service wake |
| CH340C | USB-to-UART bridge | Powered from `3V3_SYS`; V3 cap = 100 nF |
| USB ESD | ESD diode array | Must be placed close to USB-C connector, not CH340C |
| Q14, Q15 | MMBT3904 NPN | RTS/DTR auto-program circuit for ESP32 |
| ADC filter | 2.2 kΩ + 100 nF | `VOLT` → 2.2 kΩ → `VOLT_ESP` → 100 nF → GND |

USB-C configuration:
- CC1 and CC2 each have 5.1 kΩ pulldown to GND (USB-C device/sink)
- VBUS routed through resettable fuse to `VBUS_USB`
- D+/D− have 22 Ω series resistors after ESD protection

Auto-program circuit:
- `PROG_EN` → ESP32 EN/CHIP_PU (10 kΩ pull-up to `3V3_SYS`)
- `PROG_BOOT` → ESP32 IO0/BOOT (10 kΩ pull-up to `3V3_SYS`)
- Manual EN and BOOT buttons pull to GND

ESP32 local decoupling:
- 47 µF + 10 µF + 100 nF on `3V3_SYS` near the module

SD card reader details:

| SD function | Net | ESP32 GPIO | Notes |
|---|---|---:|---|
| CLK | `SD_SCK` | GPIO18 | 22 Ω series resistor |
| DAT0/DO | `SD_MISO` | GPIO19 | Direct, no series resistor |
| CMD/DI | `SD_MOSI` | GPIO23 | 22 Ω series resistor |
| DAT3/CS | `SD_CS` | GPIO13 | 22 Ω series resistor, 10 kΩ pull-up to `3V3_SYS` |

SD card powered from `3V3_SYS` with local 100 nF + 10 µF decoupling.

Regulator capacitor summary:

| Regulator | Input caps | Output caps |
|---|---|---|
| AP2112K-3.3 (`VSYS` → `3V3_SYS`) | 10 µF, 100 nF | 100 nF, 10 µF, 100 µF/10 V |
| TPL720F33 (`RAW_BAT` → `KEEP_ALIVE`) | 10 µF, 100 nF | 10 µF, 100 nF |
...
```text
Solar input connector (U47)
→ F5 1812L050PR polyfuse
→ CN3163 VIN
→ CN3163 BAT
→ BAT_BUS
→ 1812L110PR polyfuse (input protection section)
→ RAW_BAT
```

Key parameters:

- Charge current: ~330 mA (ISET = 3.6 kΩ)
- TEMP pin: tied to GND (no NTC)
- Status LEDs: CRG (charging) and DONE (full), 2.2 kΩ series resistors
- Input caps: 10 µF + 100 nF
- Output caps: 10 µF + 100 nF

Design rules:

- USB is not intended to charge the battery; USB is for programming and service only
- `BAT_BUS` is pre-fuse; all downstream loads must use `RAW_BAT`
- If the solar panel cable is long, consider additional input bulk capacitance near the connector
