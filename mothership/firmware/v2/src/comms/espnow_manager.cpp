                       ...
                       int len)
{
    ...
    //    V2 snapshots (NODE_SNAPSHOT2) are variable length; decode to the
    //    common DecodedSnapshot then convert back to node_snapshot_t so the
    //    existing queue/drain path works unchanged.
    if (isV2Snapshot(incomingBytes, len)) {
        DecodedSnapshot decoded;
        if (decodeV2(incomingBytes, len, decoded)) {
            noteNodeSampleForSyncRx(decoded.nodeId);

            node_snapshot_t snap{};
            decodedToV1(decoded, snap);
            // Fall through into the V1 handling block below by reusing the
            // same logic. Set a flag so the V1 block knows we already
            // decoded and should not re-memcpy.
            // (Implemented as a dedicated inline copy below to avoid goto
            //  complexity with the existing not_snapshot label.)
            {
                ...
                if (knownMac && (observedState == UNPAIRED || pendingBlocksAutoDeploy)) {
                    ...
                    Serial.printf("🛡️ Ignoring auto-deploy promotion from V2 snapshot for %s (state=%s)\n",
                ...
                for (auto& n : registeredNodes) {
                    if (n.nodeId == String(snap.nodeId)) {
                        ...
                        if (n.deployPending) {
                            n.deployPending = false;
                            Serial.printf("✅ Deploy pending cleared by V2 snapshot from %s\n", snap.nodeId);
                            savePairedNodes();
                        }
                        break;
                    }
                ...
                Serial.printf("📊 SNAPSHOT(V2) fw=%s seq=%lu present=0x%04X node_ts=%lu\n",
        ...
        Serial.println("[ESPNOW] V2 snapshot detected but decode failed");
        return;
    }
...
void espnow_loop() {
    ...
    // Mothership-side stale inference
    for (auto& n : registeredNodes) {
        if (!(n.state == PAIRED || n.state == DEPLOYED)) continue;
    ...
    // Periodic fleet-wide TIME_SYNC (every ~24 h)
                           ...
                           &day, &month, &year) == 6)
                {
                    ...
                    if (gSyncMode == kSyncModeDaily) {
                        ...
                        if (deployAnchor > nowUnix) {
                            deployCmd.syncPhaseUnix = deployAnchor;
                        } else {
