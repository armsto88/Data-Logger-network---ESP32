...
#else // ENABLE_BLE_GATT == 0

// Stubs when BLE is disabled

void bleSetup(const char*, const char*, BleStatusJsonProvider, BleNowUnixProvider, BleCommandRouter) {
  Serial.println("[BLE] BLE GATT disabled (ENABLE_BLE_GATT=0)");
}
...
void blePublishTelemetryEvent(const sensor_data_message_t&, const uint8_t*) {
  // no-op
}

#endif // ENABLE_BLE_GATT